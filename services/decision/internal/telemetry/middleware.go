...
func UnaryServerInterceptor(metrics *Metrics) grpc.UnaryServerInterceptor {
	...
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		metrics.DecisionsTotal.Add(ctx, 1, metric.WithAttributes(statusAttr, methodAttr))
		metrics.DecisionLatency.Record(ctx, duration, metric.WithAttributes(statusAttr, methodAttr))
